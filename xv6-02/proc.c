...
cps127(void)
{
  ...
  //this is the whole reason why we need to make this a syscall with acsses to ring 0
  acquire(&ptable.lock);

  cprintf("name \t pid \t state \t \t extpid \t ppid \t size \t cputime \n");
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    int p_id = p->ns_pid == 1 ? 0 : p->parent->ns_pid;
    //gets the namespce pid
    int local_pid = proc_pid(p);
    if(local_pid == 0)
        continue;
    if(p->state ==UNUSED)
      continue;
    if(p->state != RUNNING)
    {
        cprintf("%s \t %d \t SLEEPING \t \t %d \t %d \t %d \t %d\n" ,p->name,p->ns_pid, local_pid, p_id ,p->sz, p->cpu_time);
    }
    else
    {
